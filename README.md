...
| 子类 | 模板 | 模式 | 图位 |
|---|---|---|---|
| 基础PV \| Basic PV | 单图人物PV·基础版 / 纯文字PV·无图生成 / 单图产品PV·宣传展示 | 多模态融合 / 文生视频 | 图1=人物或产品（可选） |
| 多图分镜PV \| Multi-image Storyboard PV | 人物+产品广告PV·双参考 / 双人物同框PV·双参考 / 导演分镜PV·多镜头序列 / 首尾帧PV·强控首尾 | 多模态融合 / 首尾帧 | 图1+图2（人物+产品/双人），导演分镜可加图3场景 |
| 风格化PV \| Stylized PV | 时尚TVC快剪PV·品牌节奏 | 多模态融合 | 图1=模特，图2=产品可选 |

**通用用法**：
1. **传图**：按上表图位上传参考图；不传图时模板自动回退为**文生视频模式**（内置默认人物/产品/场景，仍可完整出 PV）。
2. **模式选择**：参考图默认「自由构图」（图只作参考不锁首帧）；需首帧与图完全一致时用「强制首帧」并保持画面比例一致（如首帧竖版选竖版）。
3. **文字控制**：默认**无字**（纯画面剪辑）；要文字就在模板文字框写——可指定位置（顶部/居中/底部/片尾卡）、语言与内容，文字自动避开人脸与产品关键细节，逐字保真。片尾品牌/Logo 文字推荐写「片尾卡」。
4. **主体优先级**：人物+产品类模板默认「模特+产品互动」；想以产品为主或以模特为主，在文字框写明即可（如「以香水瓶为主体，模特只露手」）。
5. **参数建议**：时长 10-15 秒；**先用低分辨率 0.2 测试**确认无问题再提高到 0.5/0.915；采样步数默认即可，动态强的需求调高；若关闭加速 LoRA，步数至少调到 20。
6. **LoRA**：在「资源管理 → LoRA 管理」上传（支持点击上传/拖拽/打开文件夹），上传后可改名/配展示图，之后即可在 PV 模板位置调用；加速 LoRA 勿删，删除需同步调高步数。

---

## 2026-09-04 新增：画面特效与多视角模板（15 个，全部多参模式）
> 新增顶级分类「画面特效与多视角 | VFX & Multi-View」，含 5 个子类、15 个模板，全部为**多参模式**（多参考图 <Picture 1>/<Picture 2> + {{SCENE_REF_RULE}} 自定义场景），三段式（header + integrated_multimodal_description + overall_soundscape + non_diegetic_music）标准 H3 格式。

| 子类 | 模板 |
|---|---|
| 画面切割特效 \| Split Screen | 二分屏对比特效 / 四分屏矩阵特效 / 画中画悬浮特效 |
| 人物与空间关系特效 \| Person-Space | 巨空间微缩人物 / 人物悬浮空间 / 空间透视拉伸 |
| 人物与文字排版特效 \| Typography | 3D立体文字环绕 / 文字粒子聚散 / 霓虹文字背景 |
| 人物与动漫角色互动 \| Anime Interaction | 真人×动漫并肩同行 / 真人×动漫角色对战 / 真人×动漫情感互动 |
| 同画面多视角分割 \| Multi-View | 双视角同屏分割 / 四宫格多视角监控 / 环绕多视角时间切片 |

**多参用法**：接 2 张参考图（如 <Picture 1> 真人主体 + <Picture 2> 第二主体/元素），图 3 可作为自定义场景；自定义文字类模板请在参考图/文字中提供文案内容。前端下拉自动加载（已缓存 bust），后端按「分类 > 子类 > 模板名」匹配。
...
### 3. 纯文字PV·无图生成（「补充修改」融合 PV 宣传文字）
- 模板「PV宣传片 > 基础PV > 纯文字PV·无图生成」（Text-only PV, No Image）为纯文生视频模板：无需连接任何参考图，10 秒，模式自动为「Text to Video (文生视频)【图3可选场景】」（可再接 scene_image 指定场景）。
- 选中该模板后，节点内的「补充修改 / Customization」区域会自动切换为 **PV 宣传文字引导模式**：
  - 显示引导横幅：请在输入框填写你的 **PV 宣传文字 / 文稿 / 广告语**（主角长相、服装、年龄、场景、动作、镜头、要不要屏幕文字/写什么字，越具体画面越可控）；
  - 输入框占位提示同步切换为 PV 专用文案；
  - 若未填写文字直接点「确认修改 / Apply」，输出面板会给出明确提示，不会静默跳过。
