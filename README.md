...
- **转写后询问框（双输出选项）**：直通模式下语音停止转写后，弹出「**如何输出？/ How to output?**」询问框——
  - 「⚡ 直通输出原文（不生成H3）／ Output raw text」→ **不经过 H3 生成**，直接把语音转文字原文输出给下游节点、窗口自动关闭；
  - 「⚡ 生成 H3 提示词／ Generate H3」→ 由本地大模型按 **H3 SKILL 三段式标准**扩写后再输出给下游；
  - 「✏️ 修改文字／ Edit text」→ 先编辑再手动「确定并输出」；（替换原先 3 秒自动输出）
- **输出提示词预览**：节点「补充修改」下方新增只读「**输出提示词预览 / Output Preview**」窗口，直通/语音输出的完整提示词在语音窗口关闭后仍在此显示；工作流重载时自动恢复。

**English:**

- **Confirm dialog with two output choices**: in Direct Mode, once transcription finishes, a dialog asks「如何输出？/ How to output?」—
  - 「⚡ 直通输出原文（不生成H3）／ Output raw text」→ sends the **raw transcribed text** straight to downstream nodes **without H3 generation**, then auto-closes;
  - 「⚡ 生成 H3 提示词／ Generate H3」→ expands the text into a **full H3 three-field prompt** per the H3 SKILL via the local LLM, then outputs downstream;
  - 「✏️ 修改文字／ Edit text」→ edit first, then manual "Confirm &amp; Output". (Replaces the old 3s auto-output.)
...
- **全自动流程（直通模式）**：录音时 **3 秒无声音**自动停止并转写；转写完成后弹出询问框「**如何输出？**」——「⚡ 直通输出原文」直接输出语音原文（不生成 H3）给下游并自动关闭；「⚡ 生成 H3 提示词」由本地大模型按 H3 三段式标准扩写后输出；「✏️ 修改文字」可先编辑再手动「确定并输出」。输出后可在节点下方「**输出提示词预览 / Output Preview**」只读窗口中查看发送给下游的完整提示词（窗口关闭后仍保留）。
...
- **Fully automatic flow (Direct Mode)**: while recording, **3 seconds of mic silence** auto-stops and transcribes; after transcription a dialog asks「如何输出？/ How to output?」—「⚡ 直通输出原文 / Output raw text」sends the raw transcribed text downstream **without H3 generation** and auto-closes;「⚡ 生成 H3 提示词 / Generate H3」expands it into a full H3 three-field prompt first;「✏️ 修改文字 / Edit text」lets you edit before manual "Confirm &amp; Output". The full prompt sent downstream stays visible in the node's read-only「**输出提示词预览 / Output Preview**」box even after the dialog closes.
