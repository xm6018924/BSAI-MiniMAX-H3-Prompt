...
- **转写后询问框**：直通模式下语音停止转写后，弹出「**是否修改文字？**」询问框——点击「⚡ 不修改，直接输出」即自动生成 H3 提示词并输出给下游节点、窗口自动关闭；点击「✏️ 修改文字」可先编辑再手动「确定并输出」。（替换原先 3 秒自动输出）
- **输出提示词预览**：节点「补充修改」下方新增只读「**输出提示词预览 / Output Preview**」窗口，直通/语音输出的完整提示词在语音窗口关闭后仍在此显示；工作流重载时自动恢复。

**English:**

- **Confirm dialog after transcription**: in Direct Mode, once the mic stops and the speech is transcribed, a dialog asks「是否修改文字？/ Edit text?」—「⚡ 不修改，直接输出 / Output as-is」auto-generates the H3 prompt, outputs it downstream and closes the window;「✏️ 修改文字 / Edit text」lets you edit before manual "Confirm &amp; Output". (Replaces the old 3s auto-output.)
...
- **全自动流程（直通模式）**：录音时 **3 秒无声音**自动停止并转写；转写完成后弹出询问框「**是否修改文字？**」——点击「⚡ 不修改，直接输出」即自动生成 H3 提示词并输出给下游节点、窗口自动关闭；点击「✏️ 修改文字」可先编辑再手动「确定并输出」。输出后可在节点下方「**输出提示词预览 / Output Preview**」只读窗口中查看发送给下游的完整提示词（窗口关闭后仍保留）。
...
- **Fully automatic flow (Direct Mode)**: while recording, **3 seconds of mic silence** auto-stops and transcribes; after transcription a dialog asks「**是否修改文字？/ Edit text?**」— click「⚡ 不修改，直接输出 / Output as-is」to auto-generate the H3 prompt and output to downstream, then auto-close; click「✏️ 修改文字 / Edit text」to edit first, then manual "Confirm &amp; Output". After output, the full prompt sent to downstream stays visible in the node's read-only「**输出提示词预览 / Output Preview**」box even after the dialog closes.
